...
class LSTMQuantileRegressor():
    ...
    def fit(self, x_train, y_train, x_test, y_test):
        ...
        with custom_object_scope({'quantile_loss': self.quantile_loss}):
            ...
            mc = ModelCheckpoint(f"model_quantile_is{self.q:.2f}.h5", monitor='val_loss',
                                 mode='min', save_best_only=True)
            history = self.model.fit(x_train, y_train, epochs=500, batch_size=32, validation_data=(x_test, y_test),
                                     callbacks=[es, mc])

    def predict(self, x_test):
        with custom_object_scope({'quantile_loss': self.quantile_loss}):
            best_model = load_model(f"model_quantile_is{self.q:.2f}.h5")
...
if __name__ == '__main__':
    ...
    model = LSTMQuantileRegressor(n_units=32, q=0.5)
