...
# 数据预处理
scaler_x = MinMaxScaler() # 归一化器x
scaler_y = MinMaxScaler() # 归一化器y
...
n = len(test_y)
std_error = np.sqrt(np.sum((real_y_test - real_prediction_test)**2) / (n - 1))
t_value = t.ppf((1 + confidence_level) / 2.0, n - 1)
margin_error = t_value * std_error
lower_interval = real_prediction_test - margin_error
upper_interval = real_prediction_test + margin_error
...
plt.fill_between(np.arange(len(real_y_test)), lower_interval.flatten(), upper_interval.flatten(), alpha=0.2, color='blue')
